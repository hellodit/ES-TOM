        ...
        <table class="table table-hover">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td data-label="avatar"><img alt="image"
                            src="{{empty($user->avatar) ? asset('stisla/assets/img/avatar/avatar-1.png') : asset('storage/user_photo/60/'.$user->avatar)}}"
                            class="rounded-circle" width="35" data-toggle="tooltip" title=""
                            data-original-title="{{$user->name}}"></td>
                    <td data-label="name">{{$user->name}}</td>
                    <td data-label="email">{{$user->email}}</td>
                    <td data-label="name">{{$user->histories->count()}} Kali</td>
                    <td data-label="created_at">{{$user->created_at->diffForHumans()}}</td>
                    <td data-label="Action">
                        <a href="#" class="btn btn-primary btn-sm" onclick="actControl('detail','{{$user->id}}')"
                            role="button" data-toggle="modal" data-target=".bs-modal-lg" style="cursor:pointer"><i
